...
public class SetTodoItemPriorityCommandHandlerTests
{
    ...
    [Fact]
    public async Task Handle_UpdatesPriority()
    {
        var (context, currentUser, listId, itemId) = await CreateListWithItemAsync();
        var handler = new SetTodoItemPriorityCommandHandler(context, currentUser);

        await handler.Handle(new SetTodoItemPriorityCommand(listId, itemId, PriorityLevel.High), CancellationToken.None);
    ...
    {
        var context = ApplicationDbContextFake.Create();
        var handler = new SetTodoItemPriorityCommandHandler(context, new FakeCurrentUserService());

        await Assert.ThrowsAsync<NotFoundException>(
            () => handler.Handle(new SetTodoItemPriorityCommand(999, 1, PriorityLevel.High), CancellationToken.None));
    }
    ...
    public async Task Handle_WithUnknownItemId_ThrowsNotFoundException()
    {
        ...
        await Assert.ThrowsAsync<NotFoundException>(
            () => handler.Handle(new SetTodoItemPriorityCommand(listId, 999, PriorityLevel.High), CancellationToken.None));
    }
    ...
    public async Task Handle_OnAnotherUsersList_ThrowsNotFoundException()
    {
        ...
        var handler = new SetTodoItemPriorityCommandHandler(context, new FakeCurrentUserService("someone-else"));

        await Assert.ThrowsAsync<NotFoundException>(
            () => handler.Handle(new SetTodoItemPriorityCommand(listId, itemId, PriorityLevel.High), CancellationToken.None));
    }
}
